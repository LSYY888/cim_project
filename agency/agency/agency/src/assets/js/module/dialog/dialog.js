...
define(['base'], function (base) {
	...
	BOX.prototype = {
		...
		init: function (configs) {
			...
			self.param = {
				overlayTemplate: '<div class="pub-dialog-overlay"></div>',
				temlate: '<div class="pub-dialog-box clearfix">' +
				'<div class="pub-dialog-head clearfix"><h4><a href="javascript:;" class="pub-dialog-close"><i class="icon-close"></i></a><span>{TITLE}</span></h4></div>' +
				'<div class="pub-dialog-body">{BODY}</div>' +
				'<div class="pub-dialog-foot clearfix">{FOOT}</div>' +
				'</div>',
				confirmBtnTemplate: '<button class="btn btn-primary pub-dialog-confirm">{CONFIRMTXT}</button>',
				cancelBtnTemplate: '<button class="btn btn-default pub-dialog-cancel">{CANCELTXT}</button>'
			};
			if($('.pub-dialog-overlay').length >0 && $('.pub-dialog-box').length > 0){
				return;
			}
			self.setTemplate();
		},
		...
		setTemplate: function () {
			...
			self.nodelist.box_head_node = box_node.find('.pub-dialog-head');
			self.nodelist.box_body_node = box_node.find('.pub-dialog-body');
			self.nodelist.box_foot_node = box_node.find('.pub-dialog-foot');
			self.nodelist.box_title_node = self.nodelist.box_head_node.find('span');
			self.nodelist.box_close_node = self.nodelist.box_head_node.find('.pub-dialog-close');
		...
		setPosition: function () {
			...
			var maxheight = winHeight - posTop  - (self.configs.hideTitle ? 0 : 51) - (self.configs.hideFoot ? 0 : 50);
		...
		afterShow : function(){
			...
			self.configs.onload && self.configs.onload.call(self,self, self.nodelist.box_node);
		      /*add毛玻璃效果*/
      ...
      self.configs.afterClose && self.configs.afterClose.apply(self, arguments);
		},
		/**
		...
		bindEvent: function () {
			var self = this;
			self.nodelist.box_node.on('click', '.pub-dialog-close,.pub-dialog-cancel', function (e) {
				e.preventDefault();
				self.close();
		...
		alert: function (configs) {
			...
			if(!configs.onload){
				configs.onload = function(dialog,node){
					node.find('.pub-dialog-confirm').click(function(){
					  dialog.close();
					});
				}
			}
