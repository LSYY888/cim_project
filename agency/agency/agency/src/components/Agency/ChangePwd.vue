			...
			<ul>
				<li>
					<label class="clearfix">
						<span>用户名</span>
						<input type="text" v-model:value='UserInfo.username' placeholder="请输入您的用户名" class="commone-input" ref='perUsername'/>
					</label>
				</li>
				<li>
					<label class="clearfix">
						<span>密码</span>
						<input type="text" v-model:value='UserInfo.password'   placeholder="请输入您的密码" class="commone-input" ref='perPwd'/>
					</label>
				</li>
				<li class="clearfix">
					<label class="clearfix">
						<span>手机号</span>
						<span class="commone-input" style="float: right" ref="mobile">{{userHeadinfo.mobile}}</span>
					</label>
				</li>
				<li class="identify-box">
					<label class="clearfix">
						<span>短信验证码</span>
						<input type="text" style="text-align:right" ref='perCode'/>
...
<script>
	...
	export default {
		...
		methods: {
			getUserInfo() {
				var url = this.$api.wxUrl + 'api/user/account'
           ...
           this.$http.get(url).then(response =>{
			...
			modifyinformation () {
        // 判断手机验证码是否为空  \
        var perCode = this.$refs.perCode.value
        if (perCode == '') {
        	this.messageShow = true
        ...
        if (!this.$refs.checkbox.checked) {
            this.messageShow = true
        	this.totastText = '请同意趣返返平台服务协议'
        	return
        }
        var data = {
        	vcode: perCode,
        	username: this.$refs.perUsername.value,
        	password:  this.$refs.perPwd.value,
        }
        var url = this.$api.wxUrl + 'api/user/account'
        this.$http.post(url, data).then(response => {
        	if (response.data.code === 1) {
        		this.messageShow = true
        		this.totastText = '信息修改成功'
        		setTimeout(() => {
