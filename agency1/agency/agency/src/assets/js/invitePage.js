$(function () {
  ...
  var D = {
    init: function () {
      ...
      $('.j_login_top').hide()
      $('.j_login_main').hide()
      $('.active-status-section').css('marginTop', '140px')
      var self = this
      self.bindEvent()
      ...
      if (sessionId !== '') {
        ...
        $('.j_login_main').show()
        $('.j_no_login_top').hide()
        $('.active-status-section').css('marginTop', 0)
        // 登录后所需要的接口
        //$.ajax({})
        $('.invite-people-num').text(0)
        $('.invest-money-num').text(550)
        $('.total-money-num').text(5)
        //$(".second-ul .rule-state").html('<p><i></i>好友*<span>6110</span>于2018年4月3日完成1笔投资，您获得110元现金红包奖励；</p><p><i></i>您于2018年4月2日成功邀请好友<span>*1133</span>注册返返app；</p>')
      } else {
        $('.j_no_login_top').show()
        $('.j_login_top').hide()
        $('.j_login_main').hide()
    ...
    bindEvent: function () {
      ...
      var self = this
      var tab_turn = 0
      var $_tab = $('.tab-content-section li')
      ...
      // 点击立即邀请赚500
      $('.now-invite-btn').on('click', function () {
      ...
      $_tab.on('click', function () {
        tab_turn = $_tab.index($(this))
        $(this).addClass('active').siblings().removeClass('active')
        $('.second-ul li').removeClass('show')
        $('.second-ul li').eq(tab_turn).addClass('show')
      })
    }, 
    urlFn: function (url) {
      var name = ''
